...
static char rcsid[] = "$Id: schur.c,v 1.4 1994/01/13 05:35:52 des Exp $";
...
MAT	*A, *Q;
{
	...
	if ( k_max == k_min + 1 )
	{
	    ...
	    tmp = m_entry(A,k_min,k_min) - m_entry(A,k_max,k_max);
	    /* discrim = tmp*tmp +
		4*A_me[k_min][k_max]*A_me[k_max][k_min]; */
	    discrim = tmp*tmp +
		4*m_entry(A,k_min,k_max)*m_entry(A,k_max,k_min);
	    if ( discrim < 0.0 )
	    {	/* yes -- e-vals are complex -- leave 2 x 2 block */
		k_min = k_max + 1;
		continue;
	    }
	}
	...
	while ( ! split )
	{
	    ...
	    ********************/
	    /********************
	    s = A_me[k_tmp][k_tmp] + A_me[k_max][k_max];
	    t = A_me[k_tmp][k_tmp]*A_me[k_max][k_max] -
		A_me[k_tmp][k_max]*A_me[k_max][k_tmp];
	    ********************/
	    s = a00 + a11;
	    t = a00*a11 - a01*a10;
